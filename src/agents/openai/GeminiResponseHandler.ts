...
export class OpenAIResponseHandler {
  ...
  private performWebSearch = async (query:string) : Promise<string> => {
   ...
   if(!TAVILY_API_KEY){
    throw new Error("web search is not available, Missing Tavily API key");
   }
   try{
    const response = await fetch("https://api.tavily.ai/search",{
        method: "POST",
        headers:{
    ...
    console.error(`An exception occurred while searching for query "${query}:`,error);
    return JSON.stringify({
        error : "Search failed with status"
    })
   }
  }
